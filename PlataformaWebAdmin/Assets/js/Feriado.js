...
import RegisterFeriadoInput from "../../Models/RegisterFeriadoInput.js";
import UpdateFeriadoInput from "../../Models/UpdateFeriadoInput.js";
import apiUrls from '../../Utils/ApisURL.js';

document.addEventListener("DOMContentLoaded", async function () {
  // Inicializa flatpickr en el input de fecha con localización en español
  flatpickr("#fechaNuevo", {
    dateFormat: "Y-m-d",
    locale: "es", // Establece el idioma español
  });
  flatpickr("#fechaEditar", {
    dateFormat: "Y-m-d",
    locale: "es", // Establece el idioma español
  });
...
const FeriadoModule = {
  register: async function () {
    ...
    try {
      const response = await FeriadoController.register(
        new RegisterFeriadoInput(fecha, descripcion)
      );
      ...
      alert("Ha ocurrido un error al registrar el feriado.");
    }
  },
  ...
  getall: async function () {
    ...
    const response = await FeriadoController.getall();
    const FeriadosData = response.data;
    $("#example2").DataTable({
      paging: true,
      lengthChange: true,
      searching: true,
      ...
      columns: [
        ...
        { data: "descripcion", title: "Descripción" },
        {
          ...
          render: function (data, type, row) {
            return `
                            <button type="button" class="btn bg-gradient-danger" onclick="FeriadoModule.handleDelete(${data})">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
                                <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5M8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5m3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0"/>
                                </svg>
                            </button>
                            <button type="button" class="btn bg-gradient-warning" onclick="FeriadoModule.loadingEditar(${data})">
  ...
  confirmDelete: async function () {
    try {
      const response = await FeriadoController.delete(selectedId);
      ...
      if (response.success) {
        ...
        alert("Error al eliminar el feriado: " + response.message);
      }
    } catch (error) {
  ...
  loadingEditar: async function (id) {
    ...
    const response = await FeriadoController.get(id);
    document.getElementById("fechaEditar").value = response.data[0].fecha;
  ...
  actualizar: async function () {
    try {
      ...
      const response = await FeriadoController.update(
        new UpdateFeriadoInput(selectedId, fecha, descripcion)
      );
...
window.FeriadoModule = FeriadoModule;
